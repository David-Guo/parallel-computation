...
{
    va_list ap;
    fprintf (stderr, "[scntpbmsieve-%07.2f] ", seconds ());
    va_start (ap, msg);
    vfprintf (stderr, msg, ap);
    va_end (ap);
    fputc ('\n', stderr);
...
inline static int
get (unsigned * sieve, int m)
{
    return sieve[m/32] & (1<<(m&31));
}

inline static void
set (unsigned * sieve, int m)
{
    sieve[m/32] |= (1 << (m & 31));
}
...
main (int argc, char ** argv)
{
    ...
    for (i = 1; i < argc; i++) 
    {
        if (!strcmp (argv[i], "-h"))
        {
            printf ("usage: scntpbmsieve [-h] <number>\n");
            exit (0);
        } 
